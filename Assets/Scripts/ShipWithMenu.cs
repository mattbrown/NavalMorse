...
public class ShipWithMenu : MonoBehaviour {

	public Rigidbody2D systemRB; //Player rigid body
	...
	void Start () {
//		systemRB = GetComponent<Rigidbody2D> ();
	...
	void FixedUpdate () { //fixed update is for physics calculations
		systemRB.AddForce(transform.right * currentSpeed * Time.deltaTime, ForceMode2D.Force);

		//speed limit
		if(systemRB.velocity.magnitude > currentSpeed) 
        {
         	systemRB.velocity = systemRB.velocity.normalized * maxSpeed;
        }
